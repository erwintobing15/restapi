...
class DbHandlerAgendaMou
{
    ...
    public function allAgendaMou($id, $dari_tgl, $sampai_tgl)
    {
        if ($dari_tgl == NULL and $sampai_tgl == NULL) {
            $sql = "SELECT * FROM tbl_mou ORDER BY id_surat DESC";
            $result = $this->conn->query($sql);
        } else {
            $sql = "SELECT * FROM tbl_mou 
    ...
    {
        $sql = "SELECT * FROM tbl_mou WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sql);
        if ($result->num_rows > 0) {
            header('Content-Type: application/json');
            $data = array();
                                      ...
                                      $tgl_catat,$keterangan,$id_user,$file,$file_temp)
    {
        if ($no_agenda==NULL || $no_surat==NULL || $tujuan==NULL || $isi==NULL  || $keterangan==NULL) {
            ...
            if ($this->conn->query($sql) == TRUE) {
    			move_uploaded_file($file_temp, '../../../upload/mou/'.$file); //upload file
                header('Content-Type: application/json');
    ...
    {
        $sqlcek = "SELECT * FROM tbl_mou WHERE id_surat = '".$id."'";
        $result = $this->conn->query($sqlcek); 
        while ($row = $result->fetch_assoc()) {
            if ($row['file'] != NULL) {
                unlink("../../../upload/mou/".$row['file']); // delete file
            }
        }
    ...
    {   
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "DELETE FROM tbl_mou WHERE id_surat = '".$id."'";
            $this->conn->query($sql);
